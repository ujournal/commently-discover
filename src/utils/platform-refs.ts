...
/**
 * Mastodon public post: `https://instance/@handle/{numeric-id}` on any instance.
 * Accepts trailing slashes and `.../embed` permalinks. Returns canonical post URL or null.
 */
export function getMastodonPostRef(url: string): string | null {
  try {
    const u = new URL(url);
